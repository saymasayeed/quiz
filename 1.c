...
class CourseSchedulerApp:
    ...
    def create_widgets(self):
        ...
        schedule_courses_button = tk.Button(output_frame, text="Schedule Courses", command=self.schedule_courses, bg='#0D698B', fg='#F2F1E8', bd=2, relief=tk.SOLID)
        schedule_courses_button.pack()

        refresh_button = tk.Button(output_frame, text="Refresh Output", command=self.refresh_output, bg='#0D698B', fg='#F2F1E8', bd=2, relief=tk.SOLID)
        refresh_button.pack()
    ...
    def schedule_courses(self):
        try:
            ...
            self.output_text.config(state=tk.NORMAL)
            self.output_text.insert(tk.END, f"Error connecting to database: {err}\n")
            self.output_text.config(state=tk.DISABLED)
            return
        ...
        for i in course_db:
            for j in i[-1].split(","):
                ...
                for k in range(len(day_time[0])):
                    ...
                    if len(conflicts) == 0:
                        self.output_text.config(state=tk.NORMAL)
                        self.output_text.insert(tk.END, "No Conflicts\n")
                        self.output_text.config(state=tk.DISABLED)
                        ...
                        self.output_text.config(state=tk.NORMAL)
                        self.output_text.insert(tk.END, "Conflicts\n")
                        self.output_text.config(state=tk.DISABLED)
                        ...
                        if len(room_poss) > 0:
                            ...
                            self.output_text.config(state=tk.NORMAL)
                            self.output_text.insert(tk.END, "No Rooms\n")
                            self.output_text.config(state=tk.DISABLED)
                            break
        ...
        self.output_text.insert(tk.END, mystr+"\n")
        self.output_text.insert(tk.END, "course_id, room_no for Monday, Tuesday, Wednesday, Friday\n")
        self.output_text.config(state=tk.DISABLED)
